...
class Password:
    ...
    def _check_pair(self):
        for index in range(self.length - 1):
            if self.digits[index] == self.digits[index + 1]:
                if self._check_triple(index):
                    return True
        return False

    def _check_triple(self, index):
        test = deepcopy(self.digits)
        checked_digit = test[index]
        # Remove the pair
        del test[index]
        del test[index]
        # There shouldn't be any checked_digit remaining
        return not checked_digit in test
